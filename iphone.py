...
class SnsWrapper:
    ...
    def publish_message(self, message):
        self.client.publish(TopicArn=os.getenv("TOPIC_ARN"), Message=message)
...
def handler(event, context):
    ...
    for name, part in part_by_name.items():
        r = requests.get(
            f"https://www.apple.com/shop/fulfillment-messages?pl=true&mts.0=regular&mts.1=compact&cppart=UNLOCKED/US&parts.0={part}&location=11217"
        )
        data = r.json()
